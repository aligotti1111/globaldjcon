...
// ── Resolve a user's email from auth.users by user id ─────────────────
// After the Supabase Auth migration, email lives on auth.users, not
// public.users. Client code that selects email from public.users gets null.
// This helper lets the function accept a userId and look up the real email
// server-side using the service role key.
async function resolveUserEmail(userId) {
  if (!userId) return null;
  const SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!SERVICE_KEY) {
    console.warn('[resolveUserEmail] SUPABASE_SERVICE_ROLE_KEY not set');
    return null;
  }
  try {
    ...
    if (!res.ok) {
      console.warn('[resolveUserEmail] HTTP', res.status, 'for user', userId);
      return null;
    }
...
// ── Pick the usable email: explicit wins, else resolve from userId ────
async function pickEmail(explicitEmail, userId) {
...
exports.handler = async (event) => {
  ...
  // ── 3. INBOX MESSAGE NOTIFICATION ─────────────────────────────────
    ...
    const { recipientName, recipientEmail: recipientEmailIn, recipientUserId, senderName, senderEmail, subject, message } = body;
  ...
  // ── 5. CONTACT US ─────────────────────────────────────────────────
    ...
    const { djName, djEmail: djEmailIn, djUserId, requesterName, eventDate, venueName, venueAddress, venueType, setType, startTime, endTime, equipment, notes, offerAmount, quotedRate, currency } = body;
    const djEmail = await pickEmail(djEmailIn, djUserId);
    if (!djEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve DJ email for booking_request' }) };
    }
    ...
    const { requesterName, requesterEmail: requesterEmailIn, requesterUserId, djName, status, eventDate, venueName } = body;
    const requesterEmail = await pickEmail(requesterEmailIn, requesterUserId);
    if (!requesterEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve requester email for booking_status' }) };
    }
    const dateStr = eventDate ? new Date(eventDate+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}) : '—';
    ...
    const recipientEmail = await pickEmail(recipientEmailIn, recipientUserId);
    if (!recipientEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve recipient email for booking_counter' }) };
    }
    ...
    emailPayload = {
      ...
      subject: `Counter Offer from ${escHtml(senderName || senderLabel)}`,
      html: emailTemplate(`
        <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#1a1a2e;margin-bottom:8px;">Counter Offer</h2>
        <p style="color:#666666;margin-bottom:16px;">Hi ${escHtml(recipientName || 'there')}, <strong>${escHtml(senderName || senderLabel)}</strong> has sent a counter offer for your booking at <strong>${escHtml(venueName)}</strong> on ${dateStr}.</p>
    ...
    // Mobile (event) DJ booking request: a Host booked a Mobile DJ.
    // Sends an email to the DJ (recipient).
    const { djName, djEmail: djEmailIn, djUserId, requesterName, eventDate, eventType, venueName, venueAddress, roomDetails, guestCount, packageTitle, packageCategory, startTime, endTime, cocktailNeeded, cocktailStartTime, cocktailSameRoom, totalPrice, depositAmount, depositPct, notes, isQuote, currency } = body;
    const djEmail = await pickEmail(djEmailIn, djUserId);
    if (!djEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve DJ email for mob_booking_request' }) };
    }
    const sym = {USD:'$',EUR:'€',GBP:'£',CAD:'CA$',AUD:'A$',JPY:'¥',MXN:'MX$',BRL:'R$',CHF:'Fr',SEK:'kr',NOK:'kr',DKK:'kr',NZD:'NZ$',SGD:'S$',ZAR:'R',AED:'د.إ',INR:'₹'}[currency||'USD'] || '$';
    const formatT = t => { if(!t) return ''; const [h,m]=t.split(':').map(Number); const p=h<12?'AM':'PM'; return (h%12||12)+':'+String(m).padStart(2,'0')+' '+p; };
    const dateStr = eventDate ? new Date(eventDate+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}) : '—';
    const headerLabel = isQuote ? 'New Quote Request' : 'New Booking Request';
    const subjectLabel = isQuote ? 'New Quote Request' : 'New Booking Request';
    emailPayload = {
      from: FROM, reply_to: REPLY_TO, to: [djEmail],
      subject: `${subjectLabel} from ${escHtml(requesterName)} – ${dateStr}`,
      html: emailTemplate(`
        <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#1a1a2e;margin-bottom:8px;">${headerLabel}</h2>
        <p style="color:#666666;margin-bottom:24px;">Hi ${escHtml(djName)}, you have a new ${isQuote?'quote':'booking'} request from <strong>${escHtml(requesterName)}</strong>.</p>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Date</td></tr>
          <tr><td style="color:#1a1a2e;padding-bottom:12px;font-weight:600;">${dateStr}</td></tr>
          ${eventType ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Event Type</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(eventType)}</td></tr>` : ''}
          ${packageTitle ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Package</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(packageTitle)}${packageCategory?' <span style="color:#888;font-size:12px;">('+escHtml(packageCategory)+')</span>':''}</td></tr>` : ''}
          ${venueName ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Venue</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(venueName)}${venueAddress?'<br><span style="color:#888;font-size:12px;">'+escHtml(venueAddress)+'</span>':''}${roomDetails?'<br><span style="color:#888;font-size:12px;">Room: '+escHtml(roomDetails)+'</span>':''}</td></tr>` : ''}
          ${guestCount ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Guest Count</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(String(guestCount))}</td></tr>` : ''}
          ${(startTime||endTime) ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Time</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${formatT(startTime)}${endTime?' – '+formatT(endTime):''}</td></tr>` : ''}
          ${cocktailNeeded ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Cocktail Hour</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${cocktailStartTime?'Starts '+formatT(cocktailStartTime):'Yes'}${cocktailSameRoom===false?' · separate room':''}</td></tr>` : ''}
          ${totalPrice ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">${isQuote?'Estimated Total':'Total'}</td></tr><tr><td style="color:#00b89a;font-weight:700;font-size:1.1em;padding-bottom:12px;">${sym}${Number(totalPrice).toLocaleString()} ${currency||'USD'}</td></tr>` : ''}
          ${depositAmount ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Deposit</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${sym}${Number(depositAmount).toLocaleString()}${depositPct?' ('+depositPct+'%)':''}</td></tr>` : ''}
          ${notes ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Notes</td></tr><tr><td style="color:#333333;line-height:1.6;padding-bottom:12px;">${escHtml(notes)}</td></tr>` : ''}
        </table>
        <a href="${SITE_URL}/booking-requests.html" style="display:inline-block;background:#00f5c4;color:#050507;font-weight:700;text-decoration:none;padding:14px 28px;border-radius:6px;font-family:monospace;font-size:13px;letter-spacing:.06em;text-transform:uppercase;">View ${isQuote?'Quote':'Booking'} Request</a>
      `)
    };

  } else if (type === 'booking_request_confirmation') {
    // Confirmation to the host who submitted a club DJ booking request.
    const { djName, requesterName, requesterEmail: requesterEmailIn, requesterUserId, eventDate, venueName, venueAddress, venueType, setType, startTime, endTime, equipment, venueEquipDetail, notes, offerAmount, quotedRate, currency } = body;
    const requesterEmail = await pickEmail(requesterEmailIn, requesterUserId);
    if (!requesterEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve requester email for booking_request_confirmation' }) };
    }
    const sym = {USD:'$',EUR:'€',GBP:'£',CAD:'CA$',AUD:'A$',JPY:'¥',MXN:'MX$',BRL:'R$',CHF:'Fr',SEK:'kr',NOK:'kr',DKK:'kr',NZD:'NZ$',SGD:'S$',ZAR:'R',AED:'د.إ',INR:'₹'}[currency||'USD'] || '$';
    const equipLabels = {sound_system:'Full Sound System & Decks',decks_only:'Decks/Controller Only',venue_provides:'Venue Provides All Equipment'};
    const setLabels = {opening:'Opening Set',headliner:'Headliner',closing:'Closing Set',opening_close:'Opening – Close',opening_and_closing:'Opening & Closing Set'};
    ...
    emailPayload = {
      from: FROM, reply_to: REPLY_TO, to: [requesterEmail],
      subject: `Booking Request Sent – ${escHtml(djName)} on ${dateStr}`,
      html: emailTemplate(`
        <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#1a1a2e;margin-bottom:8px;">Booking Request Sent</h2>
        <p style="color:#666666;margin-bottom:8px;">Hi ${escHtml(requesterName)}, your booking request has been sent to <strong style="color:#1a1a2e;">${escHtml(djName)}</strong>.</p>
        <p style="color:#666666;margin-bottom:24px;">You'll get an email as soon as the DJ responds.</p>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Date</td></tr>
          <tr><td style="color:#1a1a2e;padding-bottom:12px;font-weight:600;">${dateStr}</td></tr>
          ${venueName ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Venue</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(venueName)}${venueAddress?'<br><span style="color:#888;font-size:12px;">'+escHtml(venueAddress)+'</span>':''}</td></tr>` : ''}
          ${venueType ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Event Type</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${venueType==='club'?'Club':'Bar'}${setType?' · '+(setLabels[setType]||setType):''}</td></tr>` : ''}
          ${(startTime||endTime) ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Time</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${formatT(startTime)}${endTime?' – '+formatT(endTime):''}</td></tr>` : ''}
          ${equipment ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Equipment</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${equipLabels[equipment]||equipment}${venueEquipDetail?'<br><span style="color:#888;font-size:12px;">'+escHtml(venueEquipDetail)+'</span>':''}</td></tr>` : ''}
          ${quotedRate ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Quoted Rate</td></tr><tr><td style="color:#00b89a;font-weight:700;padding-bottom:12px;">${sym}${Number(quotedRate).toLocaleString()} ${currency||'USD'}</td></tr>` : ''}
          ${offerAmount ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Your Offer</td></tr><tr><td style="color:#00b89a;font-weight:700;padding-bottom:12px;">${sym}${Number(offerAmount).toLocaleString()} ${currency||'USD'}</td></tr>` : ''}
          ${notes ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Notes</td></tr><tr><td style="color:#333333;line-height:1.6;padding-bottom:12px;">${escHtml(notes)}</td></tr>` : ''}
        </table>
        <a href="${SITE_URL}/booking-requests.html" style="display:inline-block;background:#00f5c4;color:#050507;font-weight:700;text-decoration:none;padding:14px 28px;border-radius:6px;font-family:monospace;font-size:13px;letter-spacing:.06em;text-transform:uppercase;">View My Bookings</a>
      `)
    ...
    // Confirmation to the host who submitted a mobile DJ booking/quote request.
    const { djName, requesterName, requesterEmail: requesterEmailIn, requesterUserId, eventDate, packageTitle, totalPrice, depositAmount, depositPct, isQuote, currency } = body;
    const requesterEmail = await pickEmail(requesterEmailIn, requesterUserId);
    if (!requesterEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve requester email for mob_booking_confirm' }) };
    }
    const sym = {USD:'$',EUR:'€',GBP:'£',CAD:'CA$',AUD:'A$',JPY:'¥',MXN:'MX$',BRL:'R$',CHF:'Fr',SEK:'kr',NOK:'kr',DKK:'kr',NZD:'NZ$',SGD:'S$',ZAR:'R',AED:'د.إ',INR:'₹'}[currency||'USD'] || '$';
    const dateStr = eventDate ? new Date(eventDate+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}) : '—';
    const headerLabel = isQuote ? 'Quote Request Sent' : 'Booking Request Sent';
    const subjectLabel = isQuote ? 'Quote Request Sent' : 'Booking Request Sent';
    const followUp = isQuote
      ? `You'll get an email as soon as ${escHtml(djName)} sends back a custom quote.`
      : `You'll get an email as soon as ${escHtml(djName)} responds.`;
    emailPayload = {
      from: FROM, reply_to: REPLY_TO, to: [requesterEmail],
      subject: `${subjectLabel} – ${escHtml(djName)} on ${dateStr}`,
      html: emailTemplate(`
        <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#1a1a2e;margin-bottom:8px;">${headerLabel}</h2>
        <p style="color:#666666;margin-bottom:8px;">Hi ${escHtml(requesterName)}, your ${isQuote?'quote':'booking'} request has been sent to <strong style="color:#1a1a2e;">${escHtml(djName)}</strong>.</p>
        <p style="color:#666666;margin-bottom:24px;">${followUp}</p>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Date</td></tr>
          <tr><td style="color:#1a1a2e;padding-bottom:12px;font-weight:600;">${dateStr}</td></tr>
          ${packageTitle ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Package</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(packageTitle)}</td></tr>` : ''}
          ${totalPrice ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">${isQuote?'Estimated Total':'Total'}</td></tr><tr><td style="color:#00b89a;font-weight:700;font-size:1.1em;padding-bottom:12px;">${sym}${Number(totalPrice).toLocaleString()} ${currency||'USD'}</td></tr>` : ''}
          ${depositAmount ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Deposit</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${sym}${Number(depositAmount).toLocaleString()}${depositPct?' ('+depositPct+'%)':''}</td></tr>` : ''}
        </table>
        <a href="${SITE_URL}/booking-requests.html" style="display:inline-block;background:#00f5c4;color:#050507;font-weight:700;text-decoration:none;padding:14px 28px;border-radius:6px;font-family:monospace;font-size:13px;letter-spacing:.06em;text-transform:uppercase;">View My Bookings</a>
  ...
  } else if (type === 'booking_dj_confirm') {
    // Notifies the DJ that the host accepted or declined a counter offer.
    const { djName, djEmail: djEmailIn, djUserId, requesterName, status, eventDate, venueName, venueAddress, venueType, setType, startTime, endTime, equipment, notes, quotedRate, offerAmount, currency } = body;
    const djEmail = await pickEmail(djEmailIn, djUserId);
    if (!djEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve DJ email for booking_dj_confirm' }) };
    }
    const sym = {USD:'$',EUR:'€',GBP:'£',CAD:'CA$',AUD:'A$',JPY:'¥',MXN:'MX$',BRL:'R$',CHF:'Fr',SEK:'kr',NOK:'kr',DKK:'kr',NZD:'NZ$',SGD:'S$',ZAR:'R',AED:'د.إ',INR:'₹'}[currency||'USD'] || '$';
    const setLabels = {opening:'Opening Set',headliner:'Headliner',closing:'Closing Set',opening_close:'Opening – Close',opening_and_closing:'Opening & Closing Set'};
    const equipLabels = {sound_system:'Full Sound System & Decks',decks_only:'Decks/Controller Only',venue_provides:'Venue Provides All Equipment'};
    ...
    const dateStr = eventDate ? new Date(eventDate+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}) : '—';
    const statusColor = status === 'approved' ? '#3ddc84' : status === 'denied' ? '#ff5f5f' : '#ffb347';
    const statusVerb = status === 'approved' ? 'accepted' : status === 'denied' ? 'declined' : status;
    emailPayload = {
      from: FROM, reply_to: REPLY_TO, to: [djEmail],
      subject: `Counter Offer ${statusVerb.charAt(0).toUpperCase()+statusVerb.slice(1)} – ${escHtml(requesterName)}`,
      html: emailTemplate(`
        <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#1a1a2e;margin-bottom:8px;">Counter Offer ${statusVerb.charAt(0).toUpperCase()+statusVerb.slice(1)}</h2>
        <p style="color:#666666;margin-bottom:24px;">Hi ${escHtml(djName)}, <strong>${escHtml(requesterName)}</strong> has <span style="color:${statusColor};font-weight:700;">${statusVerb}</span> your counter offer for the booking on ${dateStr}.</p>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Date</td></tr>
          <tr><td style="color:#1a1a2e;padding-bottom:12px;font-weight:600;">${dateStr}</td></tr>
          ${venueName ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Venue</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(venueName)}${venueAddress?'<br><span style="color:#888;font-size:12px;">'+escHtml(venueAddress)+'</span>':''}</td></tr>` : ''}
          ${venueType ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Event Type</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${venueType==='club'?'Club':'Bar'}${setType?' · '+(setLabels[setType]||setType):''}</td></tr>` : ''}
          ${(startTime||endTime) ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Time</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${formatT(startTime)}${endTime?' – '+formatT(endTime):''}</td></tr>` : ''}
          ${equipment ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Equipment</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${equipLabels[equipment]||equipment}</td></tr>` : ''}
          ${quotedRate ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Agreed Rate</td></tr><tr><td style="color:#00b89a;font-weight:700;font-size:1.1em;padding-bottom:12px;">${sym}${Number(quotedRate).toLocaleString()} ${currency||'USD'}</td></tr>` : ''}
          ${offerAmount ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Their Offer</td></tr><tr><td style="color:#00b89a;font-weight:700;padding-bottom:12px;">${sym}${Number(offerAmount).toLocaleString()} ${currency||'USD'}</td></tr>` : ''}
          ${notes ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Notes</td></tr><tr><td style="color:#333333;line-height:1.6;padding-bottom:12px;">${escHtml(notes)}</td></tr>` : ''}
        </table>
    ...
    // Notifies a recipient (DJ or requester) that the host cancelled the booking.
    const { recipientUserId, recipientEmail: recipientEmailIn, recipientName, recipientRole, requesterName, djName, eventDate, venueName, venueAddress, venueType, setType, startTime, endTime, currency } = body;
    const recipientEmail = await pickEmail(recipientEmailIn, recipientUserId);
    if (!recipientEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve recipient email for booking_cancelled' }) };
    }
    const setLabels = {opening:'Opening Set',headliner:'Headliner',closing:'Closing Set',opening_close:'Opening – Close',opening_and_closing:'Opening & Closing Set'};
    const formatT = t => { if(!t) return ''; const [h,m]=t.split(':').map(Number); const p=h<12?'AM':'PM'; return (h%12||12)+':'+String(m).padStart(2,'0')+' '+p; };
    const dateStr = eventDate ? new Date(eventDate+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}) : '—';
    const isDjRecipient = recipientRole === 'dj';
    const greet = recipientName ? escHtml(recipientName) : 'there';
    const bodyMsg = isDjRecipient
      ? `<strong>${escHtml(requesterName)}</strong> has cancelled their booking request${venueName?' for <strong>'+escHtml(venueName)+'</strong>':''} on ${dateStr}.`
      : `Your booking request to <strong>${escHtml(djName)}</strong>${venueName?' for <strong>'+escHtml(venueName)+'</strong>':''} on ${dateStr} has been cancelled.`;
    emailPayload = {
      from: FROM, reply_to: REPLY_TO, to: [recipientEmail],
      ...
      html: emailTemplate(`
        ...
        <p style="color:#666666;margin-bottom:24px;">Hi ${greet}, ${bodyMsg}</p>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Date</td></tr>
          <tr><td style="color:#1a1a2e;padding-bottom:12px;font-weight:600;">${dateStr}</td></tr>
          ${venueName ? `<tr><td style="color:#666666;font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;padding:6px 0 2px;">Venue</td></tr><tr><td style="color:#1a1a2e;padding-bottom:12px;">${escHtml(venueName)}${venueAddress?'<br><span style="color:#888;font-size:12px;">'+escHtml(venueAddress)+'</span>':''}</td></tr>` : ''}
    ...
    // Notifies the host that the Mobile DJ has sent back a custom quote.
    const { requesterName, requesterEmail: requesterEmailIn, requesterUserId, djName, eventDate, packageTitle, quotedPrice, overtimeRate, eventHours, depositAmount, depositPct, djMessage, cocktailIncluded, cocktailPrice, currency } = body;
    const requesterEmail = await pickEmail(requesterEmailIn, requesterUserId);
    if (!requesterEmail) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Could not resolve requester email for mob_quote_response' }) };
    }
    const sym = {USD:'$',EUR:'€',GBP:'£',CAD:'CA$',AUD:'A$',JPY:'¥',MXN:'MX$',BRL:'R$',CHF:'Fr',SEK:'kr',NOK:'kr',DKK:'kr',NZD:'NZ$',SGD:'S$',ZAR:'R',AED:'د.إ',INR:'₹'}[currency||'USD'] || '$';
    const dateStr = eventDate ? new Date(eventDate+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}) : '—';
    emailPayload = {
      from: FROM, reply_to: REPLY_TO, to: [requesterEmail],
      subject: `Custom Quote from ${escHtml(djName)} – ${dateStr}`,
      html: emailTemplate(`
        <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#1a1a2e;margin-bottom:8px;">Custom Quote Received</h2>
        <p style="color:#666666;margin-bottom:16px;">Hi ${escHtml(requesterName)}, <strong>${escHtml(djName)}</strong> has sent you a custom quote for your event on ${dateStr}.</p>
        ${packageTitle ? `<p style="color:#666666;margin-bottom:16px;">Package: <strong style="color:#1a1a2e;">${escHtml(packageTitle)}</strong></p>` : ''}
        <div style="background:#f8f8f8;border-radius:8px;padding:20px;margin-bottom:24px;">
          <div style="font-size:11px;text-transform:uppercase;letter-spacing:.1em;font-family:monospace;color:#888;margin-bottom:4px;">Quoted Total</div>
          <div style="font-size:2em;font-weight:700;color:#00b89a;">${sym}${Number(quotedPrice||0).toLocaleString()} <span style="font-size:.5em;color:#888;">${currency||'USD'}</span></div>
          ${eventHours ? `<div style="margin-top:8px;color:#666;font-size:13px;">For ${escHtml(String(eventHours))} hour${eventHours==1?'':'s'} of event time</div>` : ''}
          ${overtimeRate ? `<div style="margin-top:4px;color:#666;font-size:13px;">Overtime rate: ${sym}${Number(overtimeRate).toLocaleString()}/hr</div>` : ''}
          ${cocktailIncluded ? `<div style="margin-top:4px;color:#666;font-size:13px;">Cocktail hour included${cocktailPrice?' (+'+sym+Number(cocktailPrice).toLocaleString()+')':''}</div>` : ''}
          ${depositAmount ? `<div style="margin-top:8px;color:#666;font-size:13px;">Deposit: ${sym}${Number(depositAmount).toLocaleString()}${depositPct?' ('+depositPct+'%)':''}</div>` : ''}
          ${djMessage ? `<div style="margin-top:14px;padding-top:14px;border-top:1px solid #e0e0e0;color:#333;line-height:1.6;font-size:13px;">"${escHtml(djMessage)}"</div>` : ''}
        </div>
        <a href="${SITE_URL}/booking-requests.html" style="display:inline-block;background:#00f5c4;color:#050507;font-weight:700;text-decoration:none;padding:14px 28px;border-radius:6px;font-family:monospace;font-size:13px;letter-spacing:.06em;text-transform:uppercase;">Review Quote</a>
      `)
    };
