// POST /api/rider/confirm  { riderId }
//
// Public (no login). The host opened the DJ's rider (an unguessable UUID
// capability URL, exactly like the rider VIEW page and the emailed link) and
// taps "Confirm received". We stamp booking_riders.confirmed_at — the host
// activity signal the Upcoming Bookings "New activity" sort reads — and notify
// the DJ so they know the rider was acknowledged.
//
...
export async function POST(req: Request) {
  ...
  const riderId = typeof body.riderId === 'string' && UUID_RE.test(body.riderId) ? body.riderId : null;
  if (!riderId) return NextResponse.json({ error: 'Missing rider id' }, { status: 400 });
  ...
  const { data: rData } = await db
    ...
    .eq('id', riderId)
    .maybeSingle();
  const r = rData as unknown as { id: string; booking_id: string; dj_id: string | null; confirmed_at: string | null } | null;
  if (!r) return NextResponse.json({ error: 'Rider not found.' }, { status: 404 });

  // Already confirmed — keep the first time, report success.
  if (r.confirmed_at) return NextResponse.json({ ok: true, confirmed_at: r.confirmed_at });
  ...
  // Tell the DJ the rider was confirmed.
  if (r.dj_id && process.env.RESEND_API_KEY) {
    ...
    const djEmail = await resolveUserEmail(r.dj_id);
    if (djEmail) {
      ...
      const content = `<h1 style="margin:0 0 10px;font-size:20px;color:#111;">${who} confirmed your rider</h1>
<p style="margin:0 0 16px;color:#333;font-size:15px;line-height:1.6;">${who} has confirmed they received your DJ rider${forWhen}${atVenue}.</p>
...
<a href="${SITE_URL}/upcoming-bookings" style="display:inline-block;padding:12px 28px;color:#fff;text-decoration:none;font-weight:600;font-size:14px;">Review booking</a>
      ...
      try {
        ...
        await resend.emails.send({ from: FROM, to: djEmail, subject: `${who} confirmed your rider`, html: shell(content) });
      } catch { /* non-fatal */ }
    }
  }
