...
export default function ContractSendModal({
  ...
  // Pick the club/mobile default contract text for the inline setup flow.
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        const supabase = createClient();
        const { data } = await supabase.from('users').select('dj_type').eq('id', userId).maybeSingle();
        const t = (data as { dj_type?: string | null } | null)?.dj_type;
        if (mounted && t) setText(defaultContractText(t));
      } catch { /* keep default */ }
    })();
