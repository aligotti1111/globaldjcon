...
import { defaultContractText, MOBILE_CONTRACT_TEXT } from '@/lib/contractText';
...
export default function ContractSection({ userId }: { userId: string }) {
  ...
  const [text, setText] = useState(MOBILE_CONTRACT_TEXT);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        .from('users')
        .select('docuseal_template_id, contract_file_name, contract_uploaded_at, contract_logo_url, dj_type')
        .eq('id', userId)
        .maybeSingle();
      if (!mounted) return;
      const row = data as { docuseal_template_id?: string | null; contract_file_name?: string | null; contract_uploaded_at?: string | null; contract_logo_url?: string | null; dj_type?: string | null } | null;
      ...
      setText(defaultContractText(row?.dj_type));
      setLoading(false);
    })();
