...
export default function CheckSent({
  paymentId, amount, currency, kind, alreadySettled, eventDate, venueName,
}: {
  paymentId: string; amount: number; currency: string; kind: string;
  alreadySettled: boolean; eventDate: string | null; venueName: string | null;
}) {
    ...
    <div style={wrap}>
      <div style={card}>
        ...
        {state === 'done' ? (
          ...
          <>
            <h1 style={{ fontSize: 20, margin: '0 0 8px' }}>Mailing a check?</h1>
            ...
            >
              {state === 'sending' ? 'Notifying…' : "Let my DJ know it's on the way"}
            </button>
            {state === 'error' && (
            ...
            <p style={{ color: '#6f6f80', fontSize: 12, lineHeight: 1.6, margin: '16px 0 0' }}>
              This just gives them a heads-up. Your payment is only marked received once your DJ confirms the check.
            </p>
          </>
        )}
      </div>
