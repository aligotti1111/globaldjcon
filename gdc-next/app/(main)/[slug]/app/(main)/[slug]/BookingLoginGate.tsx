...
export default function BookingLoginGate({
  ...
  // signup by default; the "Log in" link flips it. It used to route to /login,
  // which threw away the box and the page behind it. Now it swaps to the login
  // form in place, exactly like the header auth modal.
    ...
    <div className={styles.backdrop} onClick={onClose}>
      ...
      >
        ...
        {mode === 'signup' ? (
            ...
            <p className={styles.body} style={{ marginTop: '1rem', fontSize: '.78rem' }}>
              ...
              <button
                ...
                style={{
                  ...
                  color: 'var(--neon)', fontWeight: 700, fontSize: 'inherit', cursor: 'pointer',
                }}
              >
                Log in
              </button>
          ...
          <>
            {/* Login in place. On success it opens the booking form for the
                date they picked, same as a fresh signup does. */}
            <InlineLoginForm
              onDone={() => {
                if (onAuthed) onAuthed(dateKey);
                else onClose();
