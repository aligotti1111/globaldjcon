...
export interface UpcomingBooking {
  ...
  // Day-of overtime charge (mobile only). overtime_rate above is the per-hour
  // rate quoted in the contract; these are the ACTUAL overtime billed on the
  // night — hours × the rate charged, plus optional tax, and when it was
  // invoiced / paid. Independent of the deposit/balance ledger.
  overtime_hours?: number | null;
  overtime_charge_rate?: number | null;
  ...
  // Write-once history stamps for the booking log (never nulled by clear).
  overtime_invoiced_log_at?: string | null;
  overtime_paid_log_at?: string | null;
  ...
  // Write-once history stamp for the booking log (never nulled by cancel).
  contract_sent_log_at?: string | null;
  contract_signed_at?: string | null;
  contract_cancelled_at?: string | null;
  // Manual "Mark Complete" on a contract handled outside the app.
  contract_completed_at?: string | null;
  contract_completion_undone_at?: string | null;
...
export default async function UpcomingBookingsPage() {
  ...
  // Read via the admin client (getActingContext already authorized this
  // teammate/owner): the bookings table has no team-member RLS policy, so the
  ...
  const { data: rows } = await admin
    ...
    // silently deleting the row is how a DJ forgets it happened, double-books
    // the slot, or can't find the booking to look up what was agreed.
