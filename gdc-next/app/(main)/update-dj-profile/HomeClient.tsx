...
export interface HomeDj {
  ...
  country: string | null;
  zip: string | null;
  // Pre-computed home coordinates — set when the DJ saves their zip in
  // Update Profile or admin updates them. When present, the homepage
  // Near-Me sort uses these directly with no runtime Nominatim lookup.
  ...
  // Client-side caches (populated as we geocode for DJs without home_lat/lon)
...
export default function HomeClient({ initialDjs }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      // Step 1: For DJs that already have stored home_lat/home_lon (set by
      // Update Profile or admin), use those directly — no Nominatim call
      // needed. This is the fast path for properly-onboarded DJs.
      djsRef.current.forEach((dj) => {
        if (!dj._coords && dj.home_lat != null && dj.home_lon != null) {
          dj._coords = { lat: dj.home_lat, lng: dj.home_lon };
        }
      });
      // Step 2: Hydrate any localStorage-cached coords (for DJs without
      // home_lat that we've geocoded previously this session/device).
      djsRef.current.forEach((dj) => {
